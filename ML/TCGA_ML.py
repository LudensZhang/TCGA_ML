...
class ClfSet():
    ...
    def GBC(self, numOfTrees=50, maxDepth=1):
        GBC = GradientBoostingClassifier(learning_rate=0.1, n_estimators=numOfTrees, max_depth=maxDepth, random_state=5, min_samples_leaf=5)
        GBC = GBC.fit(self.X_train, self.y_train)
...
def RunClf(abundance, labels, label1, label2):
    labels = labels.apply(StageNormalize)[(labels == label1) | (labels == label2)]    # Sort by stage
    clfSet = ClfSet(np.array(abundance.loc[labels.index.tolist()]), label_binarize(labels, classes=[label1, label2]).ravel())   # Pack in class
    return(np.mean([clfSet.RFC() for i in range(10)]), np.mean([clfSet.GBCParaSelection() for i in range(10)]))
...
if __name__ == '__main__':
    ...
    abundanceTransFiles = pd.read_csv('../COAD/transcriptome/transcriptome_mRNA_fpkm.csv', index_col=0)
    labelsMicroSer = pd.read_csv('../COAD/microbiomes/metadata.csv', index_col=0)['pathologic_stage_label']
    labelsTransSer = pd.read_csv('../COAD/transcriptome/metadata.csv', index_col=0)['pathologic_stage_label']
    ...
    resultDf.append({'Data': 'transcriptome', 'Stage': 'Stage I vs Stage II', 'RFC AUROC': clfAuc[0], 'GBC AUROC': clfAuc[1]}, ignore_index=True)
    clfAuc = RunClf(abundanceTransFiles, labelsTransSer, 'Stage I', 'Stage III')
    resultDf.append({'Data': 'transcriptome', 'Stage': 'Stage I vs Stage III', 'RFC AUROC': clfAuc[0], 'GBC AUROC': clfAuc[1]}, ignore_index=True)
    clfAuc = RunClf(abundanceTransFiles, labelsTransSer, 'Stage I', 'Stage IV')
    resultDf.append({'Data': 'transcriptome', 'Stage': 'Stage I vs Stage IV', 'RFC AUROC': clfAuc[0], 'GBC AUROC': clfAuc[1]}, ignore_index=True)
    clfAuc = RunClf(abundanceTransFiles, labelsTransSer, 'Stage II', 'Stage III')
    resultDf.append({'Data': 'transcriptome', 'Stage': 'Stage II vs Stage III', 'RFC AUROC': clfAuc[0], 'GBC AUROC': clfAuc[1]}, ignore_index=True)
    clfAuc = RunClf(abundanceTransFiles, labelsTransSer, 'Stage II', 'Stage IV')
    resultDf.append({'Data': 'transcriptome', 'Stage': 'Stage II vs Stage IV', 'RFC AUROC': clfAuc[0], 'GBC AUROC': clfAuc[1]}, ignore_index=True)
    clfAuc = RunClf(abundanceTransFiles, labelsTransSer, 'Stage III', 'Stage IV')
    resultDf.append({'Data': 'transcriptome', 'Stage': 'Stage III vs Stage IV', 'RFC AUROC': clfAuc[0], 'GBC AUROC': clfAuc[1]}, ignore_index=True)
    print('Done')
